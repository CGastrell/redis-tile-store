...
RedisStore.prototype.set = function (hash, details, fn) {
  ...
  debug('SETEX "%s" ttl:%s %s', hash, store.ttl, jdetails);
  store.client.setex(phash, store.ttl, jdetails, function (er) {
    if (er) return fn(er);
...
/**
 * Delete associated hash data
 *
 * @param {String} hash
 * @api public
 */

RedisStore.prototype.delete = function (sid, fn) {
  sid = this.prefix + sid;
  debug('DEL "%s"', sid);
  this.client.del(sid, fn);
};

RedisStore.prototype.setTtl = function(minutes) {
  //convert to seconds, as needed by Redis setex
  this.ttl = 60 * minutes;
  debug("TTL set to %s seconds", this.ttl);
  //when this is called we set disableTTL to false
  this.disableTTL = false;
  debug("TTL enabled");
}
